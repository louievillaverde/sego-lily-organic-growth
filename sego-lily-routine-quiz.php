 ...
 * Description:       Sego Lily-branded skincare routine quiz. Customer answers 5 questions, gets a 2-product recommendation from Holly's line, lead syncs to Mautic with tags. Creates /your-routine page on activation. Requires LP Quiz Suite engine.
 * Version:           1.0.0
 * Author:            Lead Piranha
 ...
 * Requires Plugins:  lp-quiz-suite
 *
...
define( 'SLRQ_VERSION', '1.0.0' );
define( 'SLRQ_PLUGIN_FILE', __FILE__ );

/**
 * On activation: create the /your-routine page if it doesn't already exist.
 */
...
/**
 * Show admin notice if the LP Quiz Suite engine isn't active.
 */
add_action( 'admin_notices', 'slrq_dependency_notice' );

function slrq_dependency_notice() {
	if ( defined( 'LPRQ_VERSION' ) ) {
		return;
	}
	if ( ! current_user_can( 'activate_plugins' ) ) {
		return;
	}
	echo '<div class="notice notice-error"><p><strong>Sego Lily Routine Quiz</strong> requires the <strong>LP Quiz Suite</strong> engine plugin to be installed and activated. <a href="https://github.com/louievillaverde/lp-quiz-suite/releases/latest">Download LP Quiz Suite</a>.</p></div>';
}

/**
 * Sego Lily-specific branding filters.
 */
...
/**
 * Updater: hits this wrapper's own GitHub releases, independent of the engine's updater.
 */
...
class SLRQ_Updater {
	...
	public static function plugin_info( $result, $action, $args ) {
		...
		return (object) array(
			...
			'sections'      => array(
				'description' => 'Sego Lily-branded skincare routine quiz. Wraps the LP Quiz Suite engine with Sego Lily branding, page slug, and Holly sign-off.',
